 ...
 * (`canonical-self-claim-listener-methods.test.ts`): native-addon tests (`better-sqlite3`, and
 * this feature's own `peercred.node`) combined with real spawned children are isolated by file,
 * matching `vitest.config.ts`'s own `pool: "forks"` comment. Forks isolate each *file* into its
 * own process, so keeping the heaviest real-process tests in a smaller file of their own is the
 * same mitigation that comment already prescribes, not a new one.
 */
 ...
 * a silent `bind(2)` truncation there is a genuine, reproducible failure mode for
 * `startCanonicalSelfClaimListener`, not a load or scheduling artifact. `/tmp` is short and stable
...
/**
 * The exact, closed shape a denial crossing the real claim socket carries: `allowed` and
 * `reasonCode`, nothing else. `toEqual` against this exact object fails on any additional own
 * property — `message`, `evidence`, or anything nested inside `evidence` (a session UUID, an
 * absolute path, an image hash, a peer pid, a transcript fact) — so this is a positive proof of
 * absence, not merely a check that the two fields this test names happen to be present.
 */
const expectClosedPublicDenial = (result: Decision<unknown>, reasonCode: ReasonCode): void => {
