...
why it was never built: a deployed runtime was found carrying the reviewed Swift source while
`dist/acp-owner-auth` was absent from both that runtime and the repository, so the connector
could not be launched at all. A step that completes only when someone types it is not part of
the build.
