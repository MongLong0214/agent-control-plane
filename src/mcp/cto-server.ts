...
export const createCtoServer = (
  ...
  server.registerTool(
    ...
    async (args) => write(args.idempotencyKey, async (peer) => {
      ...
      if (args.phase === "started") {
        return respond(await cp.tasks.startWorkerExecution({
          runId: args.runId,
          taskId: args.taskId,
  ...
  server.registerTool(
    ...
    async (args) => write(args.idempotencyKey, async () => {
      // A read requested by an operator is a capacity report, not evidence that a worker
      // allocation was admitted. `startWorkerExecution` owns the WORKER_FANOUT trigger.
      if (args.refresh) await cp.capacity.refresh("DOCTOR_CAPACITY_REPORT");
