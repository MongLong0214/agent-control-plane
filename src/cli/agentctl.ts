...
export const main = async (argv: string[]): Promise<number> => {
  ...
  // Dispatched before `ACP_OPERATOR_TOKEN` is ever read, not merely before it is used (#760). A
  // claimant must not possess the owner/admin credential, and the only way to make that literally
  // true — checkable by grepping this path for the env var and finding nothing — is for the
  // branch to exist above the line that reads it, not below a client construction that happens to
  // carry an empty token for this command.
  if (command === "claim") {
...
const dispatchCanonicalSelfClaim = (args: string[], config: { databasePath: string }): Promise<number> => {
  ...
  // `--buzz-purpose` selector either: every one of those deployment facts is server-configured,
  // fixed at daemon composition time, and never read from this request.
...
const exchangeOperatorRequest = (
  ...
  // and the operator reads why the method failed rather than that the client gave up, instead of
  // this client timing out first and reporting a healthy daemon as unauthenticated or as having
  // lost its lock (#609).
 ...
 * `src/daemon/canonical-self-claim-listener.ts` (#760). Not imported from there: this file is a
 * client, never a composition root (see this file's own docstring), and the listener module's
 * other exports are daemon-side socket and kernel-credential machinery this process must never
 * construct.
 */
 ...
 * A process may prove who it is, but it cannot approve itself: the claiming connection is not
 * authorized on `ACP_OPERATOR_TOKEN` and must neither read nor require it; its authority is the
 * kernel's own record of who opened this socket, established entirely on the daemon side by
