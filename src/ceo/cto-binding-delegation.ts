...
export class CtoBindingDelegation {
  ...
  constructor(
    ...
    bindings.onSwitch((binding) => {
      if (binding.roleKey === Role.CEO) this.#requests.clear();
    });
  }
  ...
  #authorize(rawPrincipal: unknown, rawRequest: unknown): Decision<CtoBindingAuthorization> {
    ...
    // The whole permission. Holding the secret proves which session is calling; holding the
    // role is what lets that session speak here, and it is read live rather than remembered.
    const ceo = this.bindings.active(Role.CEO);
    if (!ceo || ceo.sessionId !== p.sessionId || ceo.sessionIncarnation !== authenticated.value.incarnation) return refused();
    if (authenticated.value.lifecycle !== SessionLifecycle.READY) return refused();
