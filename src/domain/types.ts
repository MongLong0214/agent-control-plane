 ...
 *
 * An ordinary replacement or a CTO-initiated switchover drains the session just as internal
 * suspendProject does, but their DRAIN_REQUEST / HANDOFF_PACKAGE expect it to leave. Acting on the
 * bare state, with no memory of which caller put it there, can consume the wrong operation. This
 * is the property that distinguishes them: set once, atomically with the DRAINING transition
 * itself (SessionRegistry.transition), by whichever of suspendProject, requestReplacement or
 * prepareSwitchover actually caused it; cleared back to null the moment the session leaves
 * DRAINING for any destination. A transition that finds the session already DRAINING is a no-op
 * (SessionRegistry.transition short-circuits before any write), so a second operation cannot
 * overwrite the first operation's cause.
 */
