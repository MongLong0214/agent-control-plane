/**
 * #833 — a peer-credential fd above int32 is refused before ToInt32 folds it back into range
 *
 ...
 */
const anAboveInt32FdCannotWrapOntoALiveOne = {
  id: "an-above-int32-fd-cannot-wrap-onto-a-live-one",
  what: "an fd above int32 cannot reach the addon, where ToInt32 would fold it back onto a live fd",
  file: "src/core/peercred.ts",
  find: "  if (!Number.isSafeInteger(fd) || fd < 0 || fd > MAX_FD) return null;\n",
  replace: "  if (!Number.isSafeInteger(fd) || fd < 0) return null;\n",
  killedBy: [
...
export default anAboveInt32FdCannotWrapOntoALiveOne;
