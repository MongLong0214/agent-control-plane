/**
 * #833 — a negative peer-credential fd is refused before ToInt32 folds it back into range
 *
 ...
 */
const aNegativeFdCannotWrapOntoALiveOne = {
  id: "a-negative-fd-cannot-wrap-onto-a-live-one",
  what: "a negative fd cannot reach the addon, where ToInt32 would fold it back onto a live fd",
  file: "src/core/peercred.ts",
  find: "  if (!Number.isSafeInteger(fd) || fd < 0 || fd > MAX_FD) return null;\n",
  replace: "  if (!Number.isSafeInteger(fd) || fd > MAX_FD) return null;\n",
  killedBy: [
...
export default aNegativeFdCannotWrapOntoALiveOne;
