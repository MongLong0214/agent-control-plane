 ...
 * frame invisible to all three counters. The comment three lines above claimed the frame was
 * "counted before anything can throw past it", which is the alibi that kept anyone from looking.
 * Measured on the named case: 4 frames counted without the record, 5 with it — the scenario
 * reconnects, so a second AUTH/OK pair is counted too.
 *
 * The cost is not one number. `framesHandled === 0` is the evidence
 * `BUZZ_MENTION_SUBSCRIBER_SILENT` fires on (`src/daemon/daemon.ts`), so a handler throwing on
 * every event presents as a quiet relay: events arriving, none counted, the subscriber diagnosed
 * as silent, and the operator sent to look at the relay while handling is what is failing.
 *
...
const aFrameWhoseSeamThrowsIsStillAFrame = {
  ...
  find: '            this.#tally.record({ rejected: "frame-handler-threw", admission: null });\n',
  replace: "",
  killedBy: [
