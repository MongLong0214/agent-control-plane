 ...
 * is absent from every deployment nobody remembered to prepare. Measured on 2026-09-21: the
 * reviewed source shipped in `generation-893d42a` while `dist/acp-owner-auth` existed in neither
 * the repository's `dist/` nor that deployed generation, so the owner-authentication connector
