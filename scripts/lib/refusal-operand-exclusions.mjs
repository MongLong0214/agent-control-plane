 ...
 * files hold **3,480** `&&`/`||` operands, against 442 in the selected files. Lifting the
 * list wholesale would grow the census roughly ninefold and require a falsifiability row or an
 * UNANSWERED reason for each of those 3,480 — which is this same boilerplate multiplied, not its
 * repair. So the list is worked down per file, and a reader deciding how to do that should see
 * 3,480 rather than derive it.
 *
 ...
 *     (3,922 at this commit); the census's own PASS line reports what it currently sees (442).
...
const reason =
  "pre-existing operands not yet answered; entered the census when selection became derived; " +
  "tracked as #833, which measured 3,480 operands across these 86 files";
